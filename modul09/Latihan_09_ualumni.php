...
$row = $result->fetch_assoc();
    ...
    //Menangani pembaruan data ketika form disubmit if($_SERVER["REQUEST_METHOD"]=="POST"){
    $id = $_POST['id'];
    $nama = $_POST['nama'];
    ...
    $check = getimagesize($_FILES["foto"]["tmp_name"]); if($check !== false){
    $uploadOk = 1;
    }else{
    ...
    //Memeriksa ukuran file (5mb maks) if($_FILES["foto"]["size"] > 5000000){
    ...
    //Jika file berhasil diunggah, tambahkan data ke
    
    $sql = "UPDATE alumni SET nama = '$nama',
     
    tahun_lulus= '$tahun_lulus', jurusan= '$jurusan', foto= '$target_file' WHERE id=$id";
    if($conn->query($sql)=== TRUE){
    echo "<div class='alert alert-success'>Data berhasil perbarui.</div>";
    }else{
    echo "<div class='alert alert-danger'>Error: "
    . $sql . "<br>" . $conn->error . "</div>";
    }
    }else{
    ...
    echo "<div class='alert alert-danger'>Error: " . $sql
    . "<br>" . $conn->error . "</div>";
    }
    }
    $conn->close();
    ...
    <input type= "number" class= "form-control" id = "tahun_lulus" name = "tahun_lulus" value= "<?php echo
    $row['tahun_lulus'];?>" required>
    </div>
    <div class = "mb-3">
